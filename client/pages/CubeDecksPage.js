...
export const CubeDecksPage = ({ cube, decks, pages, activePage, loginCallback }) => (
  <MainLayout loginCallback={loginCallback}>
    ...
    <CubeLayout cube={cube} activeLink="playtest">
      <div className="my-3">
        {pages > 1 && (
          <Paginate count={pages} active={activePage} urlF={(i) => `/cube/${cube._id}/playtest/decks/${i}`} />
        )}
        <Card>
          <CardHeader>
          ...
          <CardBody className="p-0">
            {decks.map((deck) => (
              <DeckPreview key={deck._id} deck={deck} nextURL={`/cube/${cube._id}/playtest/decks/${activePage}`} />
            ))}
          </CardBody>
        </Card>
        {pages > 1 && (
