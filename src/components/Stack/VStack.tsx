...
export const VStack: ArtisanComponent<'div', Omit<StackVariants, 'direction'>> = (props) => {
  const propsWithDefault = mergeProps({ space: 'md', align: 'center' }, props);
  const [local, others] = splitProps(propsWithDefault, ['recipe']);

  return <artisan.div class={stackRecipe(merge({ direction: 'column' }, local.recipe))} {...others} />;
};
